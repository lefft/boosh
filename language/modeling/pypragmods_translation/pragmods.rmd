---
title: "R translation of potts's `pypragmods.py` module"
subtitle: "(dependencies/imports dealt with separately)"
output: html_document
---


### 1. `pragmods.py` module description/summary docstring 
...
- Bergen, Leon; Noah D. Goodman; and Roger Levy. 2012. That's what she (could have) said: how alternative utterances affect language use. In Naomi Miyake, David Peebles, and Richard P. Cooper, eds.,
...
### 2. setup/imports for `pypragmods.py`

```{python pragmods_py_setup, eval=FALSE}
# ***tim edits here for directory reorg 
import lexica 
...
### 3. docstring for definition of class `Pragmod` 
...
### 4. definition of class `Pragmod` 

```{python pragmods_py_Pragmod_class, eval=FALSE}
class Pragmod:    
    def __init__(self,
            name="",
            lexica=None,
    ...
    def run(self,
        ...
        start_level : int (controls which listener number to begin with 
                           for display)
    ...
    def stream_lexical_uncertainty(self, n=0, display_progress=True):
        ...
        hold all the lexica in memory -- essential for 
        very large problem spaces."""        
        # If there is no lexicon prior, then this allows us to ignore it.
        lexprior_func = (lambda x : 1.0)
        # Where we have a lexicon prior, we can look up 
        # the value in self.lexprior:
    ...
    def ExpertiseListener(self, speakers):
        """Expertise listener: for each message, 
        a joint <lexicon, meaning> table"""            
...
```{python demo_pragmods_py, eval=FALSE}
...
mod = Pragmod(
  lexica=lexica,
  # Messsages and
  messages=['normal-message', 'abnormal-message', 'null'], 
  # their costs.
  costs=np.array([1.0, 2.0, 5.0]),
  # World-types and      
  states=['normal-world', 'abnormal-world'], 
  # their prior.
  ...
  temperature=3.0,
  alpha=1.0,
  # Relevant only for the anxious experts model.
  beta=1.0
) 
...
### 6. R translation of `pragmods.py` 
...
```{r pragmod_class_translation_R, eval=FALSE}

pragmod <- function(
  # what to call the model 
  name="boosh", 
  # "list of np.array with dims MxN"
  lexica=NULL, 
  # starting point for the space of lexica 
  baselexicon=NULL, 
  # iterable of length M 
  messages=c("none","some","all"), # messages=NULL, 
  # iterable of length N 
  states=c(1,2,3,4,5), # states=NULL,
  # np.array of length M (if NULL, 0 costs for all except `nullmsg` if 
  # it exists -- if so, then its cost is given by `nullcost`)
  costs=NULL, 
  # np.array of floats w length N, summing to 1. if NULL, then 
  # this becomes a uniform dist over `states` 
  prior=NULL, 
  # np.array of length length(lexica); floats summing to 1; 
  # "If no lexicon prior is given, but we do know the number
  #  of lexica (`lexcount`), then we define a flat prior over
  #  lexica. If no `lexcount` is given, we lead this undefined
  #  and the lexicon prior is implicitly flat.""
  lexprior=NULL, 
  # number of lexica (if known in advance)
  lexcount=NULL, 
  # numeric, should be positive 
  # "Usually \lambda, but lambda is a Python builtin"
  temperature=1, 
  # numeric, speaker value for the world state 
  alpha=1, 
  # numeric, speaker value for the lexicon 
  beta=1, 
  # logical, whether to assume final message is the null message 
  nullmsg=TRUE, 
  # cost of the null message if there is one 
  nullcost=5
){
  ...
  if (is.null(lexprior)){
    if (!is.null(lexcount)){
      lexprior <- rep(1, lexcount) / lexcount
    } else {
      lexprior <- 1
    }
  }
  if (is.null(costs)){
    costs <- rep(0, length(messages))
    if (nullmsg){
      costs[length(costs)] <- nullcost
    }
  }
  # initialize a length(states) x length(messages) matrix 
  final_listener <- matrix(
    data=rep(NA, length(states)*length(messages)), 
    nrow=length(states), ncol=length(messages), 
    dimnames=list(states, messages)
  )
  final_speaker <- NULL
...
run_base_model <- function(lex, n=2, display=TRUE, digits=4){
  # Basic model with a specified messages x states matrix of truth values lex
  return(run(n=n, display=display, digits=digits, 
             initial_listener=l_zero(lex), start_level=0))
}

rsa <- function(lex){
  # in original could be baselexicon or lex 
  lit <- l_zero(lex)
  spk <- speaker(lit)
  lis <- listener(spk)
  return(list(lit=lit, spk=spk, lis=lis))
}

run_uncertainty_model <- function(n=2, display=TRUE, digits=4){
  # The lexical uncertainty model of Bergen et al. 2012, 2014
  return(run(n=n, display=display, digits=digits, 
             initial_listener=uncertainty_listener(), start_level=1))
}

run_anxiety_model <- function(n=2, display=TRUE, digits=4){
  # One-shot version of the social anxiety model of Smith et al. 2013 
  return(run(n=n, display=display, digits=digits, 
             initial_listener=uncertainty_anxiety_listener(marginalize=TRUE), 
             start_level=1))
}


run <- function(initial_listener, n=2, display=TRUE, digits=4, start_level=0){
  # Generic iterator.
  #   Parameters
  #   ----------
  #     initial_listener (model function)
  #     n : int (the depth of iteration)
  #     display : bool (whether to all matrices to standard output)
  #     start_level : int (controls which listener number to 
  #                        begin with for display)
  #     digits : int (rounding precision for intermediate displays) 
  langs <- list(initial_listener)
  langs_len <- length(langs)
  
  for (idx in seq(from=1, to=(n-1)*2, by=2)){
    langs[[langs_len+1]] <- speaker(langs[[idx-1]])
    langs[[langs_len+2]] <- listener(langs[[idx]])
    
    # update counter for indexing 
    langs_len <- length(langs)
  }
  if (langs_len >= 2){
    final_speaker <- langs[[(length(langs)-1)]]
    final_listener <- langs[[length(langs)]]
  } else {
    final_speaker <- NULL
    final_listener <- langs[[length(langs)]]
  }
  ### [SOME STUFF INVOLVING DISPLAY, WHICH WE'RE NOT DOING ON FIRST RUNTHRU]
...
l_zero <- function(lex, prior){
  # maps truth-conditional lex to a probabilistic one incorporating priors 
  return(rownorm(lex * prior))
}
listener <- function(speaker){
  # same as l_zero() but transposes speaker matrix
  return(l_zero(t(speaker)))
}
speaker <- function(listener, temperature, alpha, costs){
  # bayesian speaker incorporating costs 
  return(exp(temperature * ((alpha * safelog(t(listener))) - costs)))
}
s_one <- function(lex){
  # Convenience function for S(l0(lex))
  return(speaker(l_zero(lex)))
}
l_one <- function(lex){
  # Convenience function for L(S(l0(lex)))
  return(listener(s_one(lex)))
}

uncertainty_listener <- function(lexprior, prior, lex, lexica){
  # The lexical uncertainty listener reasons over 
  # the marginal of S(L0(lex)) for all lexicons lex
  
  # [THIS NEEDS TO BE A MATRIX]
  out <- lapply(seq_along(lexica), function(idx){
    lexprior[[idx]] * prior * t(s_one(lex))
  })
  # [CHECK CORRESPONDENCE BTWN MARGIN AND AXIS IN AXIS IN ROWNORM]
  return(rownorm(apply(out, MARGIN=1, FUN=sum)))
}

lex_lik <- function(lex, lexprior, lexica){
...
### RETURN TO SIMPLE SIGNALING FOR JOINT MODELS -------
listener_lexical_marginalization <- function(lismat){
  # Return to state/message signaling by marginalizing over lexica
  
  # [CHECK CORRESPONDENCE BTWN MARGIN AND AXIS IN AXIS IN ROWNORM]
  return(apply(lismat, MARGIN=1, sum))
}

speaker_lexical_marginalization <- function(spkmat){
  # Return to state/message signaling by marginalizing over lexica
  
  # [CHECK CORRESPONDENCE BTWN MARGIN AND AXIS IN AXIS IN ROWNORM]
  return(rownorm(apply(spkmat, MARGIN=1, sum)))
}
